...
`dest` is used to proxy the [`src`](#src) path to another path, such as another URL or ZEIT Now hosted lambda.
...
<Note>
  ...
  hosted lambda, or even non-Now hosted URLs as shown in the code above. If you
  don't perform any proxying, you can safely remove{' '}
...
<Note>
  ...
  that tells ZEIT Now to build JavaScript files with Node.js and outputs them as{' '}
...
You can use this [online visual PCRE tester](https://regex101.com) to quickly test your `src` paths before deploying them to ZEIT Now.
