...
Go version 1.x is used for Go projects deployed on ZEIT Now.
...
Builders can be used with ZEIT Now by creating a `now.json` file in a project and then [using a `builds` property](/docs/v2/advanced/configuration#builds) within that configuration:
...
You can run build tasks by creating a `build` script within a `package.json` file at the entry point's level or higher. For example:
...
<Caption>
  An example <InlineCode>package.json</InlineCode> file with a{' '}
  <InlineCode>build</InlineCode> script to execute in the build step.
</Caption>
