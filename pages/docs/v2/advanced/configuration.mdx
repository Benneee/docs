...
Specifies the [ZEIT Now Platform version](/docs/v2/advanced/platform/overview#versioning) the deployment should use and is known to work with. We strongly recommend setting a `version` when working on production systems or using source control (e.g. [Git](https://git-scm.com)).
...
By setting and modifying this value, you can decide the deployment **regions** of the [Lambdas](/docs/v2/advanced/concepts/lambdas) that get created as a result of the [build steps](/docs/v2/advanced/builders). By default, the closest region to the geographical location of the deployment is used, or if using a ZEIT Now for Git integration, SFO is used by default.
