...
The ZEIT Now Platform 2.0 simplifies and streamlines the deployment of static and serverless workloads. In this guide we will walk you through the key differences between ZEIT Now 1.0 and 2.0, and the improvements this brings.
...
<Note>
  ZEIT Now 1.0 is fully maintained and supported. We will only announce a
  deprecation date once we have ensured all our customers workloads are migrated
  and the tooling is in place for a smooth transition.
</Note>
...
<Caption>
  An example <InlineCode>now.json</InlineCode> file that instructs ZEIT Now to
  use version 1.
</Caption>
...
<Caption>
  An example <InlineCode>now.json</InlineCode> file that instructs ZEIT Now to
  make the deployment source code and logs public.
</Caption>
...
With ZEIT Now 2.0 we're removing the notion of a **deployment instance**. There's no need to keep track of them and they just add overhead to the UI and database.
...
With ZEIT Now 2.0, your build steps are specified as a list of `builds` inside `now.json`.
...
<Caption>
  An example <InlineCode>now.json</InlineCode> file that instructs ZEIT Now on
  how to build the different parts of your app.
</Caption>
...
<Caption>
  An example <InlineCode>.nowignore</InlineCode> file that instructs ZEIT Now
  not to upload the <InlineCode>README.md</InlineCode> file.
</Caption>
