...
With the GitHub profile and repositories chosen for Now for GitHub to work with, the application will be ready to start using. The next step is to prepare the repository for ZEIT Now to deploy.
...
If you receive a pull request from a fork of your repository and there is either a change to the [`now.json`](/docs/v2/advanced/configuration) file or there are [secrets](/docs/v2/serverless-functions/env-and-secrets#adding-secrets) used in your project; ZEIT Now will require an authorization from you or a member of your team to deploy the pull request.
...
To stop ZEIT Now from [automatically aliasing the default branch](#aliasing-the-default-branch) to any alias setup within the `now.json` file, use the following configuration option.
...
To stop ZEIT Now from commenting on pull requests and commits, use the following configuration option in [a `now.json` file](/docs/v2/advanced/configuration) for the repository you want to stop this behavior in.
...
To stop ZEIT Now from canceling in-progress builds to prioritise building newer commits, use the following configuration option in [a `now.json` file](/docs/v2/advanced/configuration) for the repository that this behavior should exist in.
